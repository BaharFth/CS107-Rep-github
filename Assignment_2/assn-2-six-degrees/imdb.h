...
class imdb {
 ...
 public:
  ...
  const void* getActorFileAddress();

  const void* getFilmFileAddress();

  int getActorCount(); 
  int getMovieCount();

  void printActorAndMovieCounts();

  //void* Key2AddressInActorFile(int key);
  void* Key2Address(int key, const void* file);

  //void printActorName(int i);

  void printActorNames(int count);
  void printMovieNames(int count);
