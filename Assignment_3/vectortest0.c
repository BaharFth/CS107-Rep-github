...
static void PrintCharSimple(void* elem)
{
    printf("%c", *(char*)elem);
    //fflush((FILE*)fp);
}

static void printCharVector(vector* alphabet) {
    char chp; 
    for (int i = 0; i < (alphabet->logLength); i++) {
        chp = *((char*)(alphabet->elems) + i);
        printf(" %c", chp);
        chp++;
    }
}
...
static void TestAppend(vector *alphabet)
{
  ...
  fprintf(stdout, "First, here is the alphabet by VectorMap: ");
  VectorMap(alphabet, PrintChar, stdout);
  printf("\nVector Size is %d ", VectorLength(alphabet));
  //fprintf(stdout, "\nThe Letters Are: ");
  //printCharVector(alphabet);
...
static void TestAt(vector *alphabet)
{
  ...
  for (i = 0; i < 10 /*VectorLength(alphabet)*/; i += 2) { // Lowercase every other
